...
import {
  ...
  NzModalRef,
  ModalButtonOptions, ModalOptions, ModalType, OnClickCallback,
...
@Component({
  selector: 'flexible-modal',
  templateUrl: './flexible-modal.component.html',
  styleUrls: ['./flexible-modal.component.scss'],
})
export class FlexibleModalComponent<T = any, R = any> extends NzModalRef<T, R>
implements OnInit, OnChanges, AfterViewInit, OnDestroy, ModalOptions<T> {
  ...
  @ContentChild(ModalPlusHeaderDirective, { static: false })
  set modalHeader(value: ModalPlusHeaderDirective) {
    if (value && value.templateRef) {
      this.setHeaderWithTemplate(value.templateRef)
    }
  }

  @ContentChild(ModalPlusFooterDirective, { static: false })
  set modalFooter(value: ModalPlusFooterDirective) {
    if (value && value.templateRef) {
  ...
  setHeaderWithTemplate(templateRef: TemplateRef<{}>): void {
