...
export class FlexibleSizeDirective implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngOnInit(): void {
    if (!this.disabled) {
      ...
      this.renderer.addClass(this.elementRef.nativeElement, 'flexible-size');
      this.modalService.addResizeElement(this.elementRef, this.dragRef, this.scaleModeInf, this.min, this.max);
    }
  }

  private _mouseDown(e) {
    const html = getCdkGlobalOverlayWrapper(e.target);
    if (this.needDisplayTop && html && this.modalService.lastModalPlus !== html) {
      this.displayTop(html);
    }
  }

  ngAfterViewInit(): void {
    this.modalService.enableDocumentMouseListener();
  ...
  ngOnDestroy(): void {
    this.modalService.deleteResizeElement(this.elementRef);
  ...
  displayTop(el: HTMLElement) {
    this.modalService.showCurrentModalPlus(el);
  }
