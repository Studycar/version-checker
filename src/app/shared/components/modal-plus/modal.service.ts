...
class ModalBuilderForService {
  private modalRef: ComponentRef<FlexibleModalComponent> | null; // Modal ComponentRef, "null" means it has been destroyed
  ...
  getInstance(): FlexibleModalComponent | null {
    return this.modalRef && this.modalRef.instance;
  }

  getModalRef(): ComponentRef<FlexibleModalComponent> | null {
  ...
  private createModal(): void {
    ...
    this.overlayRef = this.overlay.create({
      ...
      panelClass: 'modal-plus-container'
    });
    this.modalRef = this.overlayRef.attach(new ComponentPortal(FlexibleModalComponent));
  }
...
export class ModalService {
  ...
  /** 管理弹框 */
  modalPlusContainer: Array<ModalPlusRef> = []; // 弹框容器，弹框当前弹框数量
  ...
  /** 管理可以变更尺寸元素 */
  resizeElementContainer: Array<ResizeElementInf> = [];

   // 是否激活了鼠标事件监听，变更尺寸功能需要鼠标事件协助
  isActiveMouseListener: boolean = false;
  ...
  closeAll(): void {
    this.modalPlusContainer.length = 0;
  ...
  create<T>(options: ModalOptionsForService<T> = {}): NzModalRef<T> {
    ...
    const modalBuilderer = new ModalBuilderForService(this.overlay, options, this.overlayPositionBuilder);
    const modalRefInstance = modalBuilderer.getInstance()!;
    const modalRef = modalBuilderer.getModalRef();

    this.renderer.addClass(modalRef.location.nativeElement.firstElementChild, 'aps-modal-plus');
    this.renderer.setStyle(modalRef.location.nativeElement.parentElement.parentElement, 'zIndex', Z_INDEX);

    const id = this._getId();

    this._open(id, null, modalRef);

    return modalRefInstance;
  }
