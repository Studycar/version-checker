...
import { NzMenuItemDirective, NzSubmenuService } from 'ng-zorro-antd';

// import { NzMenuItemDirective } from './nz-menu-item.directive';
// import { NzSubmenuService } from './nz-submenu.service';

@Component({
  selector: '[nz-submenu-fork]',
  exportAs: 'nzSubmenu',
  ...
  styles: [
      ...
      :root .ant-menu-submenu.ant-menu-submenu-placement-rightBottom {
        left: 4px;
        position: relative;
      }
...
export class NzSubmenuForkComponent implements OnInit, OnDestroy, AfterContentInit, OnChanges {
  ...
  @ContentChildren(NzSubmenuForkComponent, { descendants: true })
  listOfNzSubMenuComponent: QueryList<NzSubmenuForkComponent>;
  ...
  overlayPositions = [...DEFAULT_SUBMENU_POSITIONS, POSITION_MAP.rightBottom];
    ...
    combineLatest([this.nzSubmenuService.mode$, this.nzSubmenuService.open$])
      ...
      .subscribe(([mode, open]) => {
        ...
        this.overlayPositions =
          mode === 'horizontal' ? [POSITION_MAP.bottomLeft] : [POSITION_MAP.rightTop, POSITION_MAP.leftTop, POSITION_MAP.rightBottom];
        if (open !== this.nzOpen) {
