...
@Component({
  ...
  styleUrls: ['./angular-filter.component.less'],
})
export class AngularFilterComponent implements AgFilterComponent, AfterViewInit {
   ...
   * @return {FilterItem[]}
   */
  getFilterItems(isReset: boolean = false): FilterItem[] {
    ...
    const callback = (r) => {
      ...
      rows.push({
        label,
        value,
        checked: (filterRowIndex.indexOf(value.toLowerCase()) !== -1) || this.checkValue.includes(value),
      });
    }
    /** 遍历所有行，初始checkbox信息 */
  ...
  updateAllChecked(): void {
    ...
    this.checkValue = [...this.filterWords];
  ...
  setModel(model: any): void {
    if(!model) {
      // 手动重置过滤器
      this.checkValue = [];
      this.filterItems = this.getFilterItems(true);
