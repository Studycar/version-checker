...
@Component({
    ...
    selector: 'm-form-validation-messages',
...
export class FormValidationMessagesComponent implements OnInit {
    ...
    private formatErrorMessage(type: string, data: any): string {
        ...
        // 构建默认的错误信息
        const fieldDisplayName = this.appTranslationService.translate(this.displayName || this.fieldName);
        if (type === 'required') {
    ...
    ngOnInit() {
        const formControl = this.formGroup.controls[this.fieldName];
        if (!formControl) {
            throw new Error(`formControl '${this.fieldName}' not exist`);
        }
        // 监听控件状态改变
