...
export class AppApiService {
  ...
  constructor(
    ...
    this.registerOptionsFilter(options => {
      ...
      options.headers.append(
        this.appConfigService.getLanguageHeader(),
        this.appConfigService.getLanguage(),
      );
      // 设置当前时区
      options.headers.append(
        this.appConfigService.getTimezoneHeader(),
        this.appConfigService.getTimezone(),
      );
      // 附上会话Id
      options.headers.append(
        this.appConfigService.getSessionIdHeader(),
        this.appConfigService.getSessionId(),
      );
      return options;
    });
    ...
    this.registerResultFilter(response => {
      ...
      const newSessionId = response.headers.get(
        this.appConfigService.getSessionIdSetHeader(),
      );
      if (newSessionId) {
    ...
    this.setErrorConverter(error => {
      ...
      const errorContent = JSON.parse(errorMessage);
      if (errorContent) {
        this.notification.error('出错了', errorContent.message.content, {
          nzDuration: 0,
        });
      }
  ...
  // 注册全局选项过滤器
  registerOptionsFilter(
    filter: (options: RequestOptionsArgs) => RequestOptionsArgs,
  ) {
  ...
  call<T>(
    ...
    // 构建提交内容
    this.bodyFilters.forEach(h => {
      body = h(body);
    });
    options.body = body;
    if (options.method === 'POST') {
      return this.http.post(fullUrl, options.body);
    } else {
      return this.http.get(fullUrl);
    }

    // return this.http
    //   .request(options.method.toString(), fullUrl, options.body) // 提交api
    //   .publishLast()
    //   .refCount() // 防止多次subscribe导致多次提交
    //   .map(response => {
    //     // 过滤返回的结果
