...
export class AppPrivilegeService {
    ...
    translatePrivilege(privilege: string): string {
        const index = privilege.indexOf(':');
        const group = index > 0 ? privilege.substr(0, index) : 'Other';
        const name = index > 0 ? privilege.substr(index + 1) : privilege;
    ...
    isAuthorized(user: UserOutputDto, auth: AuthRequirement): PrivilegeCheckResult {
        if (user == null) {
            ...
            // 要求主租户但是用户不属于主租户时
            const errorMessage = this.appTranslationService
