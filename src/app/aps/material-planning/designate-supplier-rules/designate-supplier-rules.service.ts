...
@Injectable()
export class DesignateSupplierRulesService {
  ...
  queryUrl = '/api/ps/psDesignatesupplier/page';

  // 获取数据
  getData(dto): Observable<ResponseDto> {
    return this.http.post<ResponseDto>(`/api/ps/psDesignatesupplier/page?pageNum=${dto.pageIndex}&pageSize=${dto.pageSize}`, dto);
  }

  update(saveData: object): Observable<ResponseDto> {
    return this.http.post<ResponseDto>(
      '/api/ps/psDesignatesupplier/save',
      { ...saveData }
    );
  }

  /** 分页查询物料合格供应商列表 */
  public queryApprovedVendorList(plantCode: string, itemCode: string, vendorNumber: string, pageIndex: number, pageSize: number): Observable<ResponseDto> {
  ...
  remove(dto: object): Observable<ResponseDto> {
    return this.http.get<ResponseDto>(
      '/api/ps/psDesignatesupplier/remove', dto
    );
  }
