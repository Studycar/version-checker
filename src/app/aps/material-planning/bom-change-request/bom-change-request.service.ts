...
@Injectable()
export class BomChangeRequestService {
  ...
  getData(dto): Observable<ResponseDto> {
    return this.http.post<ResponseDto>('/api/ps/psBomEcr/page', dto);
  }

  // 根据ID获取记录信息
  getBomChangeRequestById(id: string): Observable<ResponseDto> {
    return this.http.get<ResponseDto>(
      `/api/ps/psBomEcr/queryById?id=${id}`
    );
  }

  update(saveData: object): Observable<ResponseDto> {
    return this.http.post<ResponseDto>(
      '/api/ps/psBomEcr/save',
      { ...saveData }
    );
  }
