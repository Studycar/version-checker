...
import { WorkerManagementPostsSkillEditComponent } from './edit/edit.component';

@Component({
  selector: 'worker-management-posts-skill',
  templateUrl: './posts-skill.component.html',
  providers: [PostsService]
})
export class WorkerManagementPostsSkillComponent extends CustomBaseContext
  implements OnInit {
  ...
  httpAction = { url: this.editService.querySkillUrl, method: 'POST' };
  ...
  public add(item?: any) {
    this.modal
      .static(
        ...
        {
          i: item !== undefined ? item : { POST_ID: this.i.POST_ID, SKILL_ID: null, POST_CODE: this.i.POST_CODE, POST_NAME: this.i.POST_NAME }
        },
        'lg',
      )
      .subscribe(value => {
  ...
  public remove(item: any) {
    this.editService.DeleteSkill([item[this.selectBy]]).subscribe(res => {
      if (res.Success === true) {
  ...
  removeBath() {
    ...
    this.confirmationService.confirm({
      ...
      nzOnOk: () => {
        this.editService.DeleteSkill(this.selectionKeys).subscribe(res => {
          if (res.Success === true) {
            this.msgSrv.success(
  ...
  public export() {
    super.export();
    this.editService.exportAction({ url: this.editService.exporSkillUrl, method: 'POST' }, this.getQueryParamsValue(), this.excelexport, this.context);
  }
