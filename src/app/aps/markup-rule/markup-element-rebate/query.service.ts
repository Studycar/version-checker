...
@Injectable()
export class MarkupElementRebateQueryService extends PlanscheduleHWCommonService {
  constructor(
  ...
  private baseUrl = '/api/ps/psmarkuprebate';
  ...
  private enableUrl = this.baseUrl + '/enable'; // 启用
  private disableUrl = this.baseUrl + '/disable'; // 禁用
  ...
  disable(id: string): Observable<ResponseDto> {
    return this.http.get(this.disableUrl + '/' + id);
  }

  enable(id: string): Observable<ResponseDto> {
