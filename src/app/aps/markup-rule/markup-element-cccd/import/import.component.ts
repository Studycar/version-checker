...
import { MarkupElementCccdQueryService } from '../query.service';

@Component({
  // tslint:disable-next-line:component-selector
  selector: 'markup-element-cccd-import',
  templateUrl: './import.component.html',
  providers: [MarkupElementCccdQueryService]
})
export class MarkupElementCccdImportComponent implements OnInit {

  options;

  impColumns = {
    columns: ['工厂', '存货名称', '钢种大类', '钢种', '表面', '是否受托', '长度(>=)', '长度(<)', '超长超短加价（元/张）', '生效日期', '失效日期'],
    paramMappings: [
      ...
      { field: 'lengthMin', title: '长度(>=)', columnIndex: 60, constraint: { notNull: true, } },
      { field: 'lengthMax', title: '长度(<)', columnIndex: 50, constraint: { notNull: true, } },
      { field: 'cccdMarkup', title: '超长超短加价（元/张）', columnIndex: 70, constraint: { notNull: true, } },
  ...
  expColumns = [
    ...
    { field: 'lengthMin', title: '长度(>=)', width: 150, locked: false },
    { field: 'lengthMax', title: '长度(<)', width: 150, locked: false },
    { field: 'cccdMarkup', title: '超长超短加价（元/张）', width: 150, locked: false },
  ...
  constructor(
    ...
    private queryService: MarkupElementCccdQueryService,
