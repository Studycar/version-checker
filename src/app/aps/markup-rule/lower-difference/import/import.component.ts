...
@Component({
  selector: 'lower-difference-import',
  templateUrl: './import.component.html',
  providers: [LowerDifferenceQueryService]
})
export class LowerDifferenceImportComponent implements OnInit {

  options;
  
  impColumns = {
    columns: ['工厂', '存货名称', '钢种大类', '钢种', '表面', '形式', '厚度(>=)', '厚度(<)', '下差'],
    paramMappings: [
      ...
      { field: 'prodTypeDesc', title: '形式', columnIndex: 2, constraint: { notNull: true } },
      { field: 'thicknessMin', title: '厚度(>=)', columnIndex: 5, constraint: { notNull: true, } },
      { field: 'thicknessMax', title: '厚度(<)', columnIndex: 5, constraint: { notNull: true, } },
      { field: 'lowerDifference', title: '下差', columnIndex: 5, constraint: { notNull: true, } },
  ...
  constructor(
    ...
    private queryService: LowerDifferenceQueryService,
