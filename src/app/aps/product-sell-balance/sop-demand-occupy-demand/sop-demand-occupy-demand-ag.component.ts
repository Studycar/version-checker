...
@Component({
  ...
  selector: 'product-sell-balance-sop-demand-occupy-demand-ag',
  templateUrl: './sop-demand-occupy-demand-ag.component.html',
  providers: [SopDemandOccupyService],
})
export class ProductSellBalanceSopdemandOccupyDemandAgComponent extends CustomBaseContext implements OnInit {
  ...
  public queryParams = {
    defines: [
      ...
      { field: 'category', title: '吨位', ui: { type: UiType.string } }
    ],
    values: {
      plantID: '',
  ...
  public columns = [
    ...
    {
      field: 'COUNT_LINE', headerName: '机台总数', tooltipField: 'COUNT_LINE', menuTabs: ['filterMenuTab']
    },
    {
      field: 'DIS_LINE', headerName: '占用机台数', tooltipField: 'DIS_LINE', menuTabs: ['filterMenuTab']
    },
    {
      field: 'DEMAND_QUANTITY', headerName: '需求数量',
       tooltipField: 'DEMAND_QUANTITY', menuTabs: ['filterMenuTab']
    },
    {
  ...
  setGridWidth(key: string) {
    ...
    if (columnState !== null) {
      ...
      if (this.allColumnIds.length <= 11) {
  ...
  onGridReady(params) {
    ...
    this.setGridWidth('sopdemandoccupydemand');
  }
  ...
  public loadBatch(PLANT_CODE: string, BATCH_NUMBER: string, PageIndex: number, PageSize: number) {
    // 加载版本
    this.editService.GetBatchPageList(PLANT_CODE || '', '3', BATCH_NUMBER || '', PageIndex, PageSize).subscribe(res => {
  ...
  httpAction = { url: this.editService.seachUrlDemandSum, method: 'POST' };
  public query() {
  ...
  public export() {
    super.export();
    this.commonQueryService.export({ url: this.editService.seachUrlDemandSumExp, method: 'POST' }, this.getQueryParamsValue(), this.excelexport, this.context);
