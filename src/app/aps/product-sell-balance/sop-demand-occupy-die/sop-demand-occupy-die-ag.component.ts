...
@Component({
  ...
  selector: 'product-sell-balance-sop-demand-occupy-die-ag',
  templateUrl: './sop-demand-occupy-die-ag.component.html',
  providers: [SopDemandOccupyService],
})
export class ProductSellBalanceSopdemandOccupyDieAgComponent extends CustomBaseContext implements OnInit {
  ...
  public queryParams = {
    defines: [
      ...
      { field: 'category', title: '模具', ui: { type: UiType.string } }
    ],
    values: {
      plantID: '',
  ...
  public columns = [
    ...
    {
      field: 'RESOURCE_CODE', headerName: '模具', tooltipField: 'RESOURCE_CODE', menuTabs: ['filterMenuTab']
    },
    {
      field: 'RESOURCE_DESCRIPTIONS', headerName: '模具描述', tooltipField: 'RESOURCE_DESCRIPTIONS', menuTabs: ['filterMenuTab']
    },
    {
      field: 'DIE_QTY', headerName: '模具数量', tooltipField: 'DIE_QTY', menuTabs: ['filterMenuTab']
    },
    {
  ...
  onGridReady(params) {
    ...
    this.setGridWidth('sopdemandoccupydie');
  }
  ...
  public loadBatch(PLANT_CODE: string, BATCH_NUMBER: string, PageIndex: number, PageSize: number) {
    // 加载版本
    this.editService.GetBatchPageList(PLANT_CODE || '', '4', BATCH_NUMBER || '', PageIndex, PageSize).subscribe(res => {
  ...
  httpAction = { url: this.editService.seachUrlDieSum, method: 'POST' };
  ...
  public export() {
    ...
    super.export();
    this.commonQueryService.export({ url: this.editService.seachUrlDieSumExp, method: 'POST' }, this.getQueryParamsValue(), this.excelexport, this.context);
