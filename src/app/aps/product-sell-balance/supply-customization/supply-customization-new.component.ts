...
@Component({
  ...
  selector: 'supply-customization-new',
  templateUrl: './supply-customization-new.component.html',
...
export class SupplyCustomizationNewComponent extends CustomBaseContext
  implements OnInit {
  public gridStateKey = 'supply-customization-new';
  public BtnImg = {
  ...
  public origin_columns = [
    ...
    { field: 'SALES_REGION', headerName: '大区' },
    { field: 'SALES_AREA', headerName: '区域' },
    { field: 'CUSTOMER_NAME', headerName: '客户' },
    {
      field: 'SALES_TYPE',
      headerName: '内外销',
    ...
    { field: 'DESCRIPTIONS_CN', headerName: '物料描述' },
    {
      field: 'IS_NEW',
      headerName: '是否新品',
  ...
  public loadPeriodLevel() {
    // 查询是否是周还是月
    this.innerService.getPeriodLevel(this.queryParams.values.BUSINESS_UNIT_CODE)
      .subscribe(result => {

        console.log('result.Extra' + result.data);
        this.isMothWeek = result.data;
        console.log(' this.isMothWeek' + this.isMothWeek);
    ...
    this.innerService
      ...
      .subscribe(res => {
        if (res.Extra !== undefined) {
          ...
          // 获取服务器时间
          this.innerService
            .getServerDatetime({ dateFormat: 'yyyy-MM-dd', addMonth: 1 })
            .subscribe(rsp => {
  ...
  countOtherValues_After_ValueChanged(data: any) {
    ...
    for (let i = index; i < data.data.M_N_MONTH_INVENTORY.length; i++) {
      ...
      data.data.M_N_MONTH_INVENTORY[i].UPDATE_ONHAND =
        ...
        Number.parseFloat(data.data.M_N_MONTH_INVENTORY[i].INTRANSIT_ONHAND);
      ...
      // 日均需求量=NVL(N1月销量调整,N1月销量)/当月所在天数，四舍五入取整
      if (Number.parseFloat(data.data.M_N_MONTH_INVENTORY[i].DAYS) !== 0) {
        data.data.M_N_MONTH_INVENTORY[i].AVG_DAY_REQ_QTY =
          Number.parseFloat(data.data.M_N_MONTH_INVENTORY[i].FORECAST_UPDATE) /
          Number.parseFloat(data.data.M_N_MONTH_INVENTORY[i].DAYS);
        // 小数点格式化
        data.data.M_N_MONTH_INVENTORY[
          i
        ].AVG_DAY_REQ_QTY = Math.round(Number.parseFloat(
          data.data.M_N_MONTH_INVENTORY[i].AVG_DAY_REQ_QTY,
        ));
      }

      // M+N月存销比【N1月存销比计算公式调整为：N1月末库存/N1日均需求量】
      if (Number.parseFloat(data.data.M_N_MONTH_INVENTORY[i].AVG_DAY_REQ_QTY) !== 0) {
        // data.data.M_N_MONTH_INVENTORY[i].PRODUCTION_DEPOSITRATIO =
        //   Number.parseFloat(data.data.M_N_MONTH_INVENTORY[i].UPDATE_ONHAND) /
        //   Number.parseFloat(data.data.AVG_SALES);
        data.data.M_N_MONTH_INVENTORY[i].PRODUCTION_DEPOSITRATIO =
          Number.parseFloat(data.data.M_N_MONTH_INVENTORY[i].UPDATE_ONHAND) /
          Number.parseFloat(data.data.M_N_MONTH_INVENTORY[i].AVG_DAY_REQ_QTY);
        // 小数点格式化
        data.data.M_N_MONTH_INVENTORY[
          i
  ...
  public onBusinessUnitCodeChange(eventParams: any): void {
    ...
    // 查询是否是周还是月
    // this.commonQueryService.GetPeriodLevel(this.queryParams.values.BUSINESS_UNIT_CODE)
    //   .subscribe(result => {
    //     this.isMothWeek = result.Extra;
    //   });
    this.loadPeriodLevel();
  }
  ...
  public export() {
    const action = {
      url: this.innerService.baseUrl + '/getsopsupplyforupdateNew',
      method: 'POST',
    };
        ...
        if (this.isMothWeek === 'MONTH') {
          ...
          m_n_month_inventorys.forEach(item => {
            this.columns.push({
              ...
              headerName: 'N' +
                (columnIndex + 1) + this.appTranslationService.translate('月销量'),
            });
            this.columns.push({
              field: 'M_N_MONTH_INVENTORY_MANZHU_QTY_' + columnIndex,
              headerName: 'N' +
                (columnIndex + 1) + this.appTranslationService.translate('满足数量'),
            });
            this.columns.push({
              field: 'M_N_MONTH_INVENTORY_FORECAST_UPDATE_' + columnIndex,
              headerName: 'N' +
                (columnIndex + 1) + this.appTranslationService.translate('月销量调整'),
              headerClass: 'demandCustomization_EditColumnHeader',
            ...
            this.columns.push({
              ...
              headerName: 'N' +
                (columnIndex + 1) + this.appTranslationService.translate('月规划生产量'),
              headerClass: 'demandCustomization_EditColumnHeader',
              periodIndex: columnIndex,
              // cellEditor: 'AgNumberEditor2',
              editable: true,
            ...
            this.columns.push({
              field: 'M_N_MONTH_INVENTORY_INTRANSIT_ONHAND_' + columnIndex,
              headerName: 'N' +
                (columnIndex + 1) + this.appTranslationService.translate('月预计入库量'),
            });
            this.columns.push({
              field: 'M_N_MONTH_INVENTORY_AVG_DAY_REQ_QTY_' + columnIndex,
              headerName: 'N' +
                (columnIndex + 1) + this.appTranslationService.translate('月日均需求量'),
            });
            this.columns.push({
              field: 'M_N_MONTH_INVENTORY_UPDATE_ONHAND_' + columnIndex,
              headerName: 'N' +
                (columnIndex + 1) + this.appTranslationService.translate('月末库存'),
            });
            this.columns.push({
              field: 'M_N_MONTH_INVENTORY_ONHAND_AMOUNT_' + columnIndex,
              headerName: 'N' +
                (columnIndex + 1) + this.appTranslationService.translate('月末库存金额'),
            });
            this.columns.push({
              field: 'M_N_MONTH_INVENTORY_PRODUCTION_DEPOSITRATIO_' + columnIndex,
              headerName: 'N' +
                (columnIndex + 1) + this.appTranslationService.translate('月存销比'),
            });
            monthIndex++;
            columnIndex++;
          });
          ...
          const columnFields = [
            ...
            { field: 'ONHAND_AMOUNT', headerName: '底库存金额' },
          ];
          columnFields.forEach(item => {
            // M月份为查询月份的前一个月
            const headerName =
          ...
          m_n_month_inventorys.forEach(item => {
            this.columns.push({
              field: 'M_N_MONTH_INVENTORY_FORECAST_QTY_' + columnIndex,
              headerName: 'W' +
                (columnIndex + 1) + this.appTranslationService.translate('销量'),
            });
            this.columns.push({
              field: 'M_N_MONTH_INVENTORY_MANZHU_QTY_' + columnIndex,
              headerName: 'W' +
            ...
            this.columns.push({
              field: 'M_N_MONTH_INVENTORY_FORECAST_UPDATE_' + columnIndex,
              headerName: 'W' +
                (columnIndex + 1) + this.appTranslationService.translate('销量调整'),
              headerClass: 'demandCustomization_EditColumnHeader',
              periodIndex: columnIndex,
              // cellEditor: 'AgNumberEditor2',
              editable: true,
            ...
            this.columns.push({
              field: 'M_N_MONTH_INVENTORY_PRODUCTION_N1_' + columnIndex,
              headerName: 'W' +
                (columnIndex + 1) + this.appTranslationService.translate('规划生产量'),
              headerClass: 'demandCustomization_EditColumnHeader',
              periodIndex: columnIndex,
              // cellEditor: 'AgNumberEditor2',
              editable: true,
            ...
            this.columns.push({
              field: 'M_N_MONTH_INVENTORY_INTRANSIT_ONHAND_' + columnIndex,
              headerName: 'W' +
                (columnIndex + 1) + this.appTranslationService.translate('预计入库量'),
            });
            this.columns.push({
              field: 'M_N_MONTH_INVENTORY_AVG_DAY_REQ_QTY_' + columnIndex,
              headerName: 'W' +
                (columnIndex + 1) + this.appTranslationService.translate('日均需求量'),
            });
            this.columns.push({
              field: 'M_N_MONTH_INVENTORY_UPDATE_ONHAND_' + columnIndex,
              headerName: 'W' +
                (columnIndex + 1) + this.appTranslationService.translate('末库存'),
            });
            this.columns.push({
              field: 'M_N_MONTH_INVENTORY_ONHAND_AMOUNT_' + columnIndex,
              headerName: 'W' +
                (columnIndex + 1) + this.appTranslationService.translate('末库存金额'),
            });
            this.columns.push({
              field: 'M_N_MONTH_INVENTORY_PRODUCTION_DEPOSITRATIO_' + columnIndex,
              headerName: 'W' +
                (columnIndex + 1) + this.appTranslationService.translate('存销比'),
            });
            monthIndex++;
            columnIndex++;
          });
