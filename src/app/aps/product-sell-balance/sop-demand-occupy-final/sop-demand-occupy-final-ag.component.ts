...
@Component({
  ...
  selector: 'product-sell-balance-sop-demand-occupy-final-ag',
  templateUrl: './sop-demand-occupy-final-ag.component.html',
  providers: [SopDemandOccupyService],
})
export class ProductSellBalanceSopdemandOccupyFinalAgComponent extends CustomBaseContext implements OnInit {
  ...
  public queryParams = {
    defines: [
      ...
      { field: 'resource', title: '资源', ui: { type: UiType.string } }
    ],
    values: {
      plantID: '',
  ...
  public columns = [
    ...
    {
      field: 'RESOURCE_CODE', headerName: '资源', tooltipField: 'RESOURCE_CODE', menuTabs: ['filterMenuTab']
    },
    {
      field: 'RESOURCE_DESCRIPTIONS', headerName: '资源描述', tooltipField: 'RESOURCE_DESCRIPTIONS', menuTabs: ['filterMenuTab']
    },
    {
    ...
    {
      field: 'REMAIN_DAY', headerName: '剩余天数', tooltipField: 'REMAIN_DAY', menuTabs: ['filterMenuTab']
    },
    {
      field: 'POPULATION', headerName: '人员定编', tooltipField: 'POPULATION', menuTabs: ['filterMenuTab']
    },
    {
  ...
  onGridReady(params) {
    ...
    this.setGridWidth('sopdemandoccupyfinal');
  }
  ...
  public loadBatch(PLANT_CODE: string, BATCH_NUMBER: string, PageIndex: number, PageSize: number) {
    // 加载版本
    this.editService.GetBatchPageList(PLANT_CODE || '', '1', BATCH_NUMBER || '', PageIndex, PageSize).subscribe(res => {
  ...
  httpAction = { url: this.editService.seachUrlFinalSum, method: 'POST' };
  ...
  private getQueryParamsValue(): any {
    ...
    return {
      ...
      demandDate: demandd,
      resource: this.queryParams.values.resource,
  ...
  public detailHandler(item: any) {
    this.modal
      .static(ProductSellBalanceSopdemandOccupyChildfinalAgComponent, {
        p: {
  ...
  public export() {
    ...
    super.export();
    this.commonQueryService.export({ url: this.editService.seachUrlFinalSumExp, method: 'POST' }, this.getQueryParamsValue(), this.excelexport, this.context);
