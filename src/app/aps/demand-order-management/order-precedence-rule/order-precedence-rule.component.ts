...
import { OrderPrecedenceRuleDetailComponent } from './detail/order-precedence-rule-detail.component';
import { UpdateDemandOrderManagementComponent } from './update/update-demand-order-management.component';
...
@Component({
  ...
  selector: 'order-precedence-rule',
  templateUrl: './order-precedence-rule.component.html',
  styleUrls: ['./order-precedence-rule.component.css'],
})
export class OrderPrecedenceRuleComponent extends CustomBaseContext implements OnInit {
  ...
  httpAction = {
    url: '/api/ps/pprulepriorityheader/getData',
    method: 'POST',
    data: false
  };
  ...
  add(): void {
    this.modal.static(UpdateDemandOrderManagementComponent, {
  ...
  queryDetail(item): void {
    this.modal.static(OrderPrecedenceRuleDetailComponent, { item }, 'lg', { nzWidth: 1000 }).subscribe(() => this.queryCommon());
  }
