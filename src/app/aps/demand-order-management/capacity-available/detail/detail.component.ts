...
@Component({
  selector: 'app-capacity-available-detail',
  templateUrl: './detail.component.html'
})
export class CapacityAvailableDetailComponent extends CustomBaseContext implements OnInit {
  ...
  columns = [
    ...
    { field: 'salesType', headerName: '内外销', valueFormatter: 'ctx.optionsFind(value,2).label', width: 120, menuTabs: ['filterMenuTab', 'columnsMenuTab'] },
    ...
    { field: 'demandDate', headerName: '需求日期', width: 150, menuTabs: ['filterMenuTab', 'columnsMenuTab'] },
  ...
  commonQuery() {
    this.commonQueryService.loadGridView(
      { url: '/api/ps/gopCtpCategoryUsed/queryGopCtpCategoryUsed', method: 'POST' },
      this.getQueryParams(),
      this.context
    );
  }
  ...
  getQueryParams() {
    return {
      ...
      capacityCategory: this.data.capacityCategory,
