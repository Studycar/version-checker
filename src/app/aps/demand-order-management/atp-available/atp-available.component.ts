...
import { AtpAvailableDetailComponent } from './detail/detail.component';

@Component({
  selector: 'app-atp-available',
  templateUrl: './atp-available.component.html'
})
export class AtpAvailableComponent extends CustomBaseContext implements OnInit {
  ...
  commonQuery() {
    this.commonQueryService.loadGridView(
      { url: '/api/ps/gopAtpOnhand/queryGopAtpOnhand', method: 'GET' },
      this.getQueryParams(),
      this.context
    );
  }
