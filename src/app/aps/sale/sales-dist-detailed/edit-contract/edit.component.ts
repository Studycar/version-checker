...
@Component({
  selector: 'sales-dist-detailed-contract-edit',
  templateUrl: './edit.component.html',
  providers: [SalesDistDetailedQueryService]
})
export class SalesDistDetailedContractEditComponent implements OnInit {
  ...
  isTransfer: Boolean = false; // 分货类型是否为调拨单
  saveMethod: string = 'save';
  isShowCusSearch: Boolean = false; // 是否展示客户搜索框
  ...
  isDistTypeDisabled: Boolean = true; // 分货类型是否不能修改
  isBranchDisabled: Boolean = true; // 分行客户订单是否不能修改
  isPlaceDisabled: Boolean = true; // 目的地是否不能修改
  isHouboDisabled: Boolean = true; // 厚薄料是否不能修改
  isCusDisabled: Boolean = true; // 客户简称是否不能修改
  isContractDisabled: Boolean = true; // 合同号是否不能修改
  isTransDisabled: Boolean = true; // 运输方式是否不能修改
  ...
  ngOnInit() {
    ...
    this.loadOptions();
    if(!this.isTransfer) {
      this.saveMethod = 'transferOrderBilled';
    }
  }

  setView() {
    // 现货分货 或 调拨单已开单 显示客户搜索框
    if(this.isCurrent || (this.i.distType === 'DBD' && this.i.distDetailedState === '40')) this.isShowCusSearch = true;
  }
  ...
  verifyEdit() {
    if(!this.i.salesOrderCode) {
      if(this.isCurrent) {
        this.isDistTypeDisabled = false;
        this.isPlaceDisabled = false;
        this.isHouboDisabled = false;
        this.isCusDisabled = false;
        this.isContractDisabled = false;
        this.isTransDisabled = false;
      } else if(this.i.distDetailedState === '20') {
        this.isDistTypeDisabled = false;
        this.isPlaceDisabled = false;
        this.isHouboDisabled = false;
        ...
        this.isContractDisabled = false;
        this.isTransDisabled = false;
      } else if(this.i.distDetailedState === '40') {
        this.isDistTypeDisabled = false;
        this.isPlaceDisabled = false;
        this.isHouboDisabled = false;
        this.isContractDisabled = false;
    ...
    // 调拨单已开单：允许编辑客户
    if (this.i.distDetailedState === '40' && this.i.distType === 'DBD') {
      this.isCusDisabled = false;
    }
  }
  ...
  save(value) {
    if(this.iClone.contractCode === this.i.contractCode) {
      this.queryService[this.saveMethod](this.i).subscribe(res => {
        if(res.code === 200) {
  ...
  saveOnhands(data) {
    this.i.batchCode = data.batchCode;
    this.i.onHandQuantitiesId = data.id;
  }
