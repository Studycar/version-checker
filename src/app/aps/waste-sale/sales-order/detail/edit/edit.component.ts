...
export class SalesOrderDetailEditWasteComponent implements OnInit {
  ...
  ngOnInit(): void {
    if(this.i.id) {
      ...
      this.editService.getDetailedOne(this.i.id).subscribe(res => {
        if(res.code === 200) {
          this.i = Object.assign({}, this.i, res.data);
      ...
      // this.i.detailedNum = this.editService.generateCode2('XSDMX',4);
      this.i.priceByWeight = '0';
    ...
    if(cusOrderCode !== '') {
      ...
      .subscribe(res => {
        if(res.data.content.length > 0) {
          ...
          this.clearCusOrder();
          this.msgSrv.info(this.appTranslationService.translate('客户订单号无效'));
        }
      });
    } else {
      this.clearCusOrder();
  ...
  public loadStocks(
    ...
    if(!this.contract.contractCode) {
      this.msgSrv.warning(this.appTranslationService.translate('请先选择合同号！'));
      return;
    }
    this.editService
      .getOnhandsWasteSale({
        ...
        batchCode: stockCode,
        steelGrade: this.contract.steelType,
        itemCode: this.contract.attribute3,
        grade: this.contract.attribute5,
        surface: this.contract.surface,
        filterBatchCode: 'Y', // 过滤废料销售订单明细已存在批号
        pageIndex: pageIndex,
        pageSize: pageSize,
      })
      .subscribe(res => {
  ...
  saveOnhands(data) {
    // const quantitySy = (this.contract.quantitySy || this.contract.quantitySy == 0) ? this.contract.quantitySy : this.contract.quantity;
    // if(decimal.minus(data.avaQuantity, quantitySy) > 0) {
    //   this.msgSrv.error(this.appTranslationService.translate(`存货数量${data.avaQuantity}大于合同待分货量${quantitySy}，请重新匹配`));
    //   return;
    // }
    ...
    this.i.standardsType = data.standardType;
  ...
  saveContract(data) {
    ...
    this.i.basePrice = decimal.roundFixed(data.basePrice, 6);
    this.i.deposit = data.deposit;
    this.i.depositRatio = data.depositRatio;
    this.contract = data;
    // 基价更新时：更新限价
    this.generateFixedPrice();
  }
  ...
  generateAmountIncludingTax() {
    ...
    const poundsLost = this.i.poundsLost || 0;
    this.i.amountIncludingTax = decimal.roundFixed(decimal.mul(poundsLost, fixedPrice), 2);
    // 含税金额更新时：更新税额、金额
