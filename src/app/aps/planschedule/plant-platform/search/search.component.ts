...
@Component({
  ...
  providers: [PlantPlatformService]
})
export class PlantPlatformSearchComponent implements OnInit {
  ...
  private GetPlantPlatformQueryConditions() {
    this.plantPlatformService.GetPlantPlatformQueryConditions().subscribe(res => {
      if (res.code === 200) {
        this.plantPlatformQueryConditionsOutputDto = res.data;
  ...
  private GetScheduleGroupCodes() {
    ...
    this.commonQuery.GetUserPlantGroup(inputDto.PlantCode, inputDto.ScheduleRegionCode).subscribe(res => {
      if (res.Success === true) {
        const outPutDto = res.Extra;
        if (outPutDto.length > 0) {
          this.groupCodes = [];
          outPutDto.forEach(groupCode => {
            this.groupCodes.push({ value: groupCode.scheduleGroupCode, label: `${groupCode.scheduleGroupCode}(${groupCode.descriptions})` });
          });

          if (this.searchResult !== undefined && (this.searchResult.ScheduleGroupCode !== undefined
            || this.searchResult.ScheduleGroupCode !== null)) {
  ...
  public groupChange(value: any) {
    if (this.scheduleGroupResourcesInputDto.ScheduleGroupCode === undefined) {
      this.scheduleGroupResourcesInputDto.ScheduleGroupCode = null;
    }

    this.plantPlatformService.GetScheduleGroupResources(this.scheduleGroupResourcesInputDto).subscribe(res => {
      if (res.code === 200) {
        this.scheduleGroupResourcesOutputDto = res.data;
        this.treeDataTable = [];
