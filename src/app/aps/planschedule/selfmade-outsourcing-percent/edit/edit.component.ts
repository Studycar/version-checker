...
export class SelfmadeoutsourcingpercentEditComponent implements OnInit {
  ...
  constructor(
    ...
    public selfmadeOutsourcingPercentManageService: SelfmadeOutsourcingPercentManageService,
 ...
 */
    if (this.i.id != null) {
      this.isModify = true;
      this.selfmadeOutsourcingPercentManageService.Get(this.i.id).subscribe(result => {
        this.i = result.data;
...
*/
  selectMaterial(e: any): void {
    ...
    this.selfmadeOutsourcingPercentManageService.loadMaterials(e, this.i.plantCode)
  ...
  change1({ sender, event, Text }) {
    ...
    if (value === '') {
      ...
      this.commonQueryService.getUserPlantItemPageList(this.i.plantCode || '', Text || '', '', 1, sender.PageSize).subscribe(res => {
        ...
        const selectMaterialItem = res.data.content.find(x => x.itemId === Text);
  ...
  save() {
    ...
    if (this.i.itemId === undefined || this.i.itemId === '') {
      this.msgSrv.warning(this.appTranslationService.translate('物料无效'));
      return;
    }
    let sum = this.i.selfmadePercent + this.i.outsourcingPercent;
    if (sum !== 100) {
      this.msgSrv.error(this.appTranslationService.translate('自制比例和外发比例之和必须为100'));
      return;
    }

    this.selfmadeOutsourcingPercentManageService.Edit(this.i).subscribe(res => {
      if (res.code === 200) {
