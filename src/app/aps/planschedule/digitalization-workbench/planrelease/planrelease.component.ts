...
@Component({
  ...
  selector: 'planschedule-digitalization-workbench-planrelease',
  templateUrl: './planrelease.component.html',
  // styleUrls: ['../../../../../assets/css/common.css'],
  providers: [EditService],
})
export class PlanscheduleDigitalizationWorkbenchPlanReleaseComponent implements OnInit {
  ...
  confirm() {
    ...
    this.editService.PlanRelease(
      ...
      this.i.similar_flag ? 'Y' : 'N',
      this.i.remark || ''
    ).subscribe(result => {
      if (result !== null && result.code === 200) {
        this.msgSrv.success(this.appTranslationService.translate(result.msg || '计划发布成功'));
      } else {
        this.msgSrv.warning(this.appTranslationService.translate(result.msg || '计划发布失败'));
      }
      // 完成loading
