...
import { CustomerDemandChangeOrderQueryService } from './query.service';

@Component({
  selector: 'customer-demand-change-order',
  templateUrl: './demand-change-order.component.html',
  providers: [CustomerDemandChangeOrderQueryService]
})
export class CustomerDemandChangeOrderComponent extends CustomBaseContext implements OnInit {
  ...
  constructor(
    ...
    public queryService: CustomerDemandChangeOrderQueryService,
  ...
  queryParams = {
    defines: [
      ...
      { field: 'changeOrderState', readonly: true, title: '变更单状态', ui: { type: UiType.select, options: this.cusOrderStateOptions } },
  ...
  type: string = 'changeApproval';
  handleCancel() {
    this.isVisible = false;
    this.productCategory = null;
  }
  handleOk() {
    this.queryService.changeApproval(this.productCategory).subscribe(res => {
      if(res.code === 200) {
  ...
  changeApproval() {
    this.isVisible = true;
    this.type = 'changeApproval';
  }

  applyChange() {
    this.isVisible = true;
    this.type = 'applyChange';
  }
