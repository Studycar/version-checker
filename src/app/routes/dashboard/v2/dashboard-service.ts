...
export class DashboardService {
  constructor(private appTrans: AppTranslationService) {}

  /**
   * 获取订单完成量数据
   */
  _getOrderCompletion(resArr) {
    // 渐变起点
    var pointStart = [
    ...
    // 渐变终点
    var pointEnd = [0.5 - 0.5 * Math.sin(1), 0.5 + 0.5 * Math.cos(1)];
    const thisObj = this;
    var options = {
      backgroundColor: '#fff',
      title: {
        text: thisObj.appTrans.translate('单位：台/件'),
        left: 'right',
        top: '10%',
        textStyle: {
          color: '#919CA3',
      ...
      tooltip: {
        ...
        formatter: function(params, ticket) {
          var res;
          if (params.name === thisObj.appTrans.translate('年度')) {
            res =
              '<div><p>' +
              thisObj.appTrans.translate('年度总数量：') +
              resArr.Result[0].YEAR_TOTAL +
              '</p></div>';
            res +=
              '<div><p>' +
              thisObj.appTrans.translate('年度完成数：') +
              resArr.Result[0].YEAR_FINISH_TOTAL +
              '</p></div>';
            res +=
              '<div><p>' +
              thisObj.appTrans.translate('年度完成率：') +
              resArr.Result[0].YEAR_FINISH_RATE +
              '%</p></div>';
          } else if (params.name === thisObj.appTrans.translate('月度')) {
            res =
              '<div><p>' +
              thisObj.appTrans.translate('月度总数量：') +
              resArr.Result[0].MONTH_TOTAL +
              '</p></div>';
            res +=
              '<div><p>' +
              thisObj.appTrans.translate('月度完成数：') +
              resArr.Result[0].MONTH_FINISH_TOTAL +
              '</p></div>';
            res +=
              '<div><p>' +
              thisObj.appTrans.translate('月度完成率：') +
              resArr.Result[0].MONTH_FINISH_RATE +
              '%</p></div>';
          } else if (params.name === thisObj.appTrans.translate('年度未满足率')) {
            res =
              '<div><p>' +
              thisObj.appTrans.translate('年度总数量：') +
              resArr.Result[0].YEAR_TOTAL +
              '</p></div>';
            res += `<div><p>${thisObj.appTrans.translate('年度未完成数：')}${resArr
              .Result[0].YEAR_TOTAL -
              resArr.Result[0].YEAR_FINISH_TOTAL} </p></div>`;
            res +=
              '<div><p>' +
              thisObj.appTrans.translate('年度未完成率：') +
              (100 - resArr.Result[0].YEAR_FINISH_RATE).toFixed(2) +
              '%</p></div>';
          } else if (params.name === thisObj.appTrans.translate('月度未满足率')) {
            res =
              '<div><p>' +
              thisObj.appTrans.translate('月度总数量：') +
              resArr.Result[0].MONTH_TOTAL +
              '</p></div>';
            res += `<div><p>${thisObj.appTrans.translate(
              '月度未完成数：',
            )} ${resArr.Result[0].MONTH_TOTAL -
              resArr.Result[0].MONTH_FINISH_TOTAL} </p></div>`;
            res +=
              '<div><p>' +
              thisObj.appTrans.translate('月度未完成率：') +
              (100 - resArr.Result[0].MONTH_FINISH_RATE).toFixed(2) +
              '%</p></div>';
          }
          return res;
        },
      ...
      legend: {
        ...
        formatter: function(item) {
          if (item === thisObj.appTrans.translate('年度')) {
            return `${thisObj.appTrans.translate('年度')} ${
              resArr.Result[0].YEAR_TOTAL
            }`;
          } else {
            return `${thisObj.appTrans.translate('月度')} ${
              resArr.Result[0].MONTH_TOTAL
            }`;
          }
        },
        data: [
          {
            name: thisObj.appTrans.translate('年度'),
            icon: 'rect',
          },
          {
            name: thisObj.appTrans.translate('月度'),
            icon: 'rect',
          },
        ],
        padding: [7, 9],
      ...
      series: [
        {
          name: thisObj.appTrans.translate('占比'),
          type: 'pie',
          ...
          data: [
            ...
            {
              name: thisObj.appTrans.translate('年度未满足率'),
        ...
        },
        {
          ...
          data: [
            ...
            {
              name: thisObj.appTrans.translate('月度未满足率'),
   ...
   */
  _getOrderProductionCycle(res, barWidth = 20, rotateVal = 40) {
    var params = {
      _XArr: [],
      _YArr: [],
    ...
    var maxValue =
      Math.max(..._YArr) > STANDARD ? Math.max(..._YArr) : STANDARD;
    var options = {
      // color:colorList,
      backgroundColor: '#fff',
      title: {
        text: thisObj.appTrans.translate('单位：天'),
        left: 'right',
        top: '10%',
        textStyle: {
          color: '#919CA3',
      ...
      series: [
        {
          markLine: {
            ...
            data: [
              {
                name: thisObj.appTrans.translate('Y 轴值为 100 的水平线'),
                yAxis: STANDARD,
              },
            ],
          },
   ...
   * 获取按时完成率
   */
  _getOrderCompletionRate(res) {
    const thisObj = this;
    var { YEAR_TOTAL, FINISH_RATE, UNFINISH_RATE, CANCEL_RATE } = res.Result[0];
    var echartData = [
      {
        value: FINISH_RATE,
        name: thisObj.appTrans.translate('已完成'),
      },
      {
        value: UNFINISH_RATE,
        name: thisObj.appTrans.translate('未完成'),
      },
      {
        value: CANCEL_RATE,
        name: thisObj.appTrans.translate('已取消'),
      },
    ];
    var rich = {
      total: {
    ...
    var options = {
      ...
      legend: {
        ...
        formatter: name => {
          return (
            '{total|' +
            thisObj.appTrans.translate('订单完成量') +
            '\n' +
            YEAR_TOTAL +
            '}'
          );
      ...
      series: [
        {
          name: thisObj.appTrans.translate('订单完成量'),
          type: 'pie',
          radius: [78, 95],
          label: {
            normal: {
    ...
    var option = {
      ...
      tooltip: {
        ...
        formatter: function(params) {
          var tooltipStr =
            '<p>' +
            thisObj.appTrans.translate('工单总个数 : ') +
            totalArr[params[0].dataIndex] +
            '<p>' +
            thisObj.appTrans.translate('标准齐套率 : ') +
            standardArr[params[0].dataIndex] +
            ' %' +
            '<p>' +
            thisObj.appTrans.translate('扩展齐套率 : ') +
            finishArr[params[0].dataIndex] +
            ' %' +
            '<p>' +
            thisObj.appTrans.translate('日期 : ') +
      ...
      series: [
        {
          name: thisObj.appTrans.translate('标准齐套率'),
          smooth: true,
          type: 'line',
          symbolSize: 3,
          symbol: 'circle',
        ...
        {
          name: thisObj.appTrans.translate('扩展齐套率'),
          smooth: true,
          type: 'line',
          symbolSize: 8,
          symbol: 'circle',
   ...
   * 作业完成率
   */
  _getWorkChangeCompletionRate(res) {
    const thisObj = this;
    var _params = {
      _XArr: [],
      detail: [],
    ...
    var options = {
      backgroundColor: '#fff',
      title: {
        text: thisObj.appTrans.translate('作业完成率'),
        left: 'center',
        top: '4%',
        textStyle: {
          color: '#919CA3',
      ...
      tooltip: {
        ...
        trigger: 'item',
        formatter: function(params) {
          if (params.name === thisObj.appTrans.translate('标准线')) {
            return;
          }
          var _res = '';
          for (let item of res.Result[0].DATA[params.dataIndex]
            .MO_FINISH_DETAIL) {
            if (item.KEY === thisObj.appTrans.translate('完成率')) {
      ...
      series: [
        {
          markLine: {
            ...
            data: [
              {
                name: thisObj.appTrans.translate('标准线'),
                yAxis: STANDARD,
              },
            ],
          },
  ...
  _getWorkCompletionRate(res) {
    ...
    var options = {
      backgroundColor: '#fff',
      title: {
        text: thisObj.appTrans.translate('作业变动率'),
        left: 'center',
        top: '4%',
        textStyle: {
          color: '#919CA3',
  ...
  _getGanter(res, timeRange) {
    ...
    var options = {
      ...
      tooltip: {
        formatter: function(val) {
          ...
          str +=
            plantGroup === ''
              ? ''
              : thisObj.appTrans.translate('计划组：') +
                plantGroup +
                '</br><div style="height: 10px;width: 100%;"></div>';
          str +=
            resource === ''
              ? ''
              : thisObj.appTrans.translate('资源：') +
                resource +
                '</br><div style="height: 10px;width: 100%;"></div>';
          str +=
            date === ''
              ? ''
              : thisObj.appTrans.translate('时间：') +
                date +
                '</br><div style="height: 10px;width: 100%;"></div>';
          str +=
            resourceType === ''
              ? ''
              : thisObj.appTrans.translate('资源类型：') +
                resourceType +
                '</br><div style="height: 10px;width: 100%;"></div>';
          str +=
            resourceAvailableTime === ''
              ? ''
              : thisObj.appTrans.translate('资源可用量（小时）：') +
                resourceAvailableTime +
                '</br><div style="height: 10px;width: 100%;"></div>';
          str +=
            resourceWorkingTime === ''
              ? ''
              : thisObj.appTrans.translate('资源使用量（小时）：') +
                resourceWorkingTime +
                '</br><div style="height: 10px;width: 100%;"></div>';
          str +=
            value === ''
              ? ''
              : thisObj.appTrans.translate('资源利用率：') + value + '%';
          return str;
        },
      },
      ...
      legend: function(dom) {
        ...
        var str =
          '<ul style="margin: 0; padding: 0; list-style: none; font-size: 12px; font-family: 微软雅黑; color: #434D54;">';
